...
function init(ctx) {
  ...
  timeago.analyzeData = function analyzeData (sbx) {
  ...
  timeago.updateVisualisation = function updateVisualisation(sbx) {
    ...
    var gpsDisplay = timeago.analyzeData(sbx);
    var infos = [{
      label: 'URL'
      , value: '<a href=\"' + gpsDisplay.url + '\" target=\"_blank\" rel=\"noopener noreferrer\">' + gpsDisplay.position + ' ' + gpsDisplay.accuracy + '</a>'
    }];
    sbx.pluginBase.updatePillText(timeago, {
      value: 'GPS',
      label: gpsDisplay.position + ' ' + gpsDisplay.accuracy,
      info: infos
